...
Las **organizaciones** de Docker Hub requieren un plan de pago, asi que el equipo
no usa una. Cada integrante publica en su **cuenta personal gratuita**, con los
repositorios en **publico**.

Esto no genera ningun problema: las imagenes publicas se descargan **sin iniciar
sesion**, por lo que las VM de produccion pueden hacer `pull` de cualquiera de
ellas. En el `docker-compose` se escribe el nombre completo de la imagen.

| Imagen | Tag | Quien la publica | Estado |
|--------|-----|------------------|--------|
| `osomar/ms-residentes` | `0.1.0` | @Osomar1705 | **Publicada** |
| `osomar/ms-usuarios` | `0.1.0` | @Osomar1705 | **Publicada** |
| `<cuenta>/ms-pagos` | `0.1.0` | @sebastianperez72 | Pendiente |
| `<cuenta>/ms-incidencias` | `0.1.0` | @fabianbot1331 | Pendiente |
| `<cuenta>/ms-ficha-residente` | `0.1.0` | @Brisseth-raton | Pendiente |
| `<cuenta>/ms-analitico` | `0.1.0` | @carloscondor1610 | Pendiente |
| `<cuenta>/ingesta01..03` | `0.1.0` | @carloscondor1610 | Pendiente |

Se descarta usar una unica cuenta compartida: obligaria a que los seis usen la
misma contrasena, y una cuenta personal no permite accesos individuales.
